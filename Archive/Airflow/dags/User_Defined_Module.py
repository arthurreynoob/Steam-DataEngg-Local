...
def get_steamspy_request(url,parameters=None,page=None):
    ...
    if response:
        ...
        #saved to parquet
        data_pandas.to_parquet(f"data/SteamSpy/{date_name}/spark_{parameters[value1]}_{page}_steamspy.parquet",index=False)
...
def steamspy_download_all_page(page_start=0,page_end=61):
    """
    ...
    for page in range(page_start,page_end+1):
        ...
        data = get_steamspy_request(url,params,page)
...
def process_batches(app_list, download_folder, file_name, batchsize = 100, pause = 1, batch_num=1):
    """
    Process app data in batches, writing directly to file

    app_list: app list cotaining appid and name
    download_folder: location where to store data
    file_name: file name
    ...
    for batch in app_list_pyarrow.iter_batches(batch_size = batchsize):
        
        print(f'Downloading batch {batch_num}, {batchsize} apps')
        
        batch_df = batch.to_pandas() 
        
        date_name = dt.datetime.today().strftime("%B")
        
        directory = f'{download_folder}/{date_name}'
        os.makedirs(directory, exist_ok=True)
...
def prepare_index(index_folder,index_filename,app_list,batch):
    ...
    date_name = date_name = dt.datetime.today().strftime("%y-%m-%d-%X")
...
def steamapi_start(folder = "data/steamapi", file_name = 'steamapi'):
    ...
    try:
        app_list,batch_num=read_index(f'{folder}/{month}/remaining_apps','index.txt')
        ...
        app_list = 'data/app_list.parquet'
